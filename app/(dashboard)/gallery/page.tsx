...
export default function GalleryPage() {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    if (!formTitle.trim() || !formFile) {
      setNotification({ type: "error", message: "All fields are required, including an image file." });
      return;
    }
    ...
    try {
      ...
      if (!res.success || !res.data) {
        throw new Error(res.message || "Failed to add image to gallery");
      }

      setImages((prev) => [res.data!, ...prev]);
      setNotification({ type: "success", message: "Image uploaded and published successfully!" });
      setTimeout(() => {
        closeModals();
      }, 1000);
    } catch (err: any) {
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotification({ type: "success", message: "Gallery image updated successfully!" });
      setTimeout(() => {
        closeModals();
      }, 1000);
    } catch (err: any) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 py-4 px-2 sm:px-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-gradient-to-r from-slate-900 to-indigo-950 p-6 sm:p-8 rounded-3xl shadow-xl text-white relative overflow-hidden">
        ...
        <div className="space-y-2 z-10">
          ...
          <p className="text-slate-300 max-w-xl text-sm sm:text-base">
            Upload, update, and manage official images. All uploads sync directly to the public website showcase.
          </p>
        </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 space-y-4">
        <div className="flex flex-col lg:flex-row lg:items-center gap-4">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search images by title keyword..."
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-slate-500 mt-1 max-w-sm mx-auto text-sm">
            We couldn&apos;t find any gallery items matching your criteria. Try uploading a new image.
          </p>
                  ...
                  <div className="relative aspect-video w-full overflow-hidden bg-slate-100">
                    ...
                    <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                      <button
                        ...
                        className="p-3 bg-white/90 hover:bg-white text-slate-800 rounded-full hover:scale-110 transition-all shadow-md"
                        title="Zoom Image"
                      >
                      ...
                      <button
                        ...
                        className="p-3 bg-white/90 hover:bg-white text-indigo-600 rounded-full hover:scale-110 transition-all shadow-md"
                        title="Edit Details"
                      >
                      ...
                      <button
                        ...
                        className="p-3 bg-white/90 hover:bg-white text-rose-600 rounded-full hover:scale-110 transition-all shadow-md"
                        title="Delete Image"
                      >
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-[4px] flex items-center justify-center z-50 p-4 overflow-y-auto">
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900">
                      {isAddOpen ? "Add New Image" : "Update Gallery Item"}
                    </h3>
                    <p className="text-xs text-slate-400">
                      {isAddOpen ? "Upload a photo to publish on the website gallery." : `Modify details for asset ID: ${editImage?.id}`}
                    </p>
                  </div>
                </div>
                <button
              ...
              <form onSubmit={isAddOpen ? handleAddSubmit : handleUpdateSubmit} className="p-6 space-y-5 flex-1">
                {/* Title */}
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider">Image Title</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider">Image File</label>
                  
                  {formFilePreview ? (
                    <div className="relative aspect-video rounded-2xl overflow-hidden border border-slate-200 group bg-slate-50">
                      <img
                        src={formFilePreview}
                        alt="Preview"
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button
                          ...
                          className="bg-rose-600 text-white font-bold px-3 py-1.5 rounded-xl text-xs flex items-center gap-1 shadow-md hover:bg-rose-700 transition-colors"
                        >
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 mt-0.5">Supports PNG, JPG, JPEG, WEBP files up to 5MB.</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-50">
                  <button
                    ...
                    className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-xl text-xs transition-colors disabled:opacity-50"
                  >
                    Discard
                  </button>
      ...
      <AnimatePresence>
        {lightboxImage && (
          <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-[2px] flex flex-col justify-between z-50 p-4">
            ...
            <div className="flex items-center justify-between text-white p-2">
              ...
              <button
                ...
                className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-full transition-colors"
                title="Close"
              >
                <X className="h-5 w-5" />
              </button>
            ...
            {/* Main Image View */}
            <div className="flex-1 flex items-center justify-center p-2 sm:p-6 max-h-[80vh]">
              <img
            ...
            <div className="text-center text-white pb-6 space-y-1.5 max-w-xl mx-auto px-4">
              ...
              <p className="text-xs text-slate-500 font-semibold flex items-center justify-center gap-1">
                ...
                Uploaded on: {new Date(lightboxImage.createdAt).toLocaleDateString()}
              </p>
            </div>
            ...
            >
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-slate-400 leading-relaxed max-w-xs mx-auto">
                  Are you sure you want to delete this image? It will be removed from Cloudinary storage and the public website permanently.
                </p>
              </div>
